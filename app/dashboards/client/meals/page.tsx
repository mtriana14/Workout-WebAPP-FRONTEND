...
export default function MyMealsPage() {
  ...
  const handleCreatePlan = (event: React.FormEvent) => {
    ...
    if (!userId) {
      setStatusMessage("Sign in before creating a meal plan.");
      return;
    }

    if (!form.name.trim()) {
      setStatusMessage("Meal plan name is required.");
      return;
    }

    const plan = clientDashboardService.createMyMealPlan(userId, {
      name: form.name,
  ...
  const updateLocalStatus = (plan: ClientMealPlan, status: ClientMealPlan["status"]) => {
    ...
    const updated = clientDashboardService.updateMyMealPlan(userId, plan.meal_plan_id, { status });
    if (!updated) {
      return;
    }

    setPlans((current) => current.map((item) => (item.meal_plan_id === updated.meal_plan_id ? updated : item)));
  ...
  const deleteLocalPlan = (plan: ClientMealPlan) => {
    ...
    clientDashboardService.deleteMyMealPlan(userId, plan.meal_plan_id);
    setPlans((current) => current.filter((item) => item.meal_plan_id !== plan.meal_plan_id));
    setSelectedPlan(null);
    setStatusMessage("Meal plan deleted.");
  };
      ...
      <main className="hh-dash-main">
        <div className="hh-dash-content">
          <div>
            ...
            <p className="hh-page-subtitle">Create your own meal plans or view plans from your coach</p>
          </div>
          ...
          <form className="hh-card" onSubmit={handleCreatePlan} style={{ display: "grid", gridTemplateColumns: "1fr 1.5fr auto", gap: 12, alignItems: "end" }}>
            ...
            <div className="hh-field">
              <label className="hh-field__label">Meal Plan Name</label>
              <input className="hh-input" value={form.name} onChange={(event) => setForm((current) => ({ ...current, name: event.target.value }))} placeholder="High Protein Week" />
            </div>
            <div className="hh-field">
              <label className="hh-field__label">Plan Details</label>
              <input className="hh-input" value={form.description} onChange={(event) => setForm((current) => ({ ...current, description: event.target.value }))} placeholder="Breakfast, lunch, dinner, macros, notes" />
            </div>
