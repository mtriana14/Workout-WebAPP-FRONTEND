...
export default function FindCoachesPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        setLoading(true);
        const [coachesData, pendingData, myCoachData] = await Promise.all([
          clientDashboardService.getCoaches(),
          clientDashboardService.getPendingRequest(userId),
          clientDashboardService.getMyCoach(userId),
        ]);
        setCoaches(coachesData.coaches);
        setPendingRequest(pendingData.pending_request);
        setHasCoach(!!myCoachData.coach);
      } catch {
  ...
  const handleSendRequest = async () => {
    ...
    setSending(true);
    try {
      await clientDashboardService.sendRequest(
        userId,
      ...
      setRequestMessage("");
      // Recargar el pending request
      const pendingData =
        await clientDashboardService.getPendingRequest(userId);
      setPendingRequest(pendingData.pending_request);
    } catch (error) {
      showToast(
  ...
  const isBlocked = !!pendingRequest || hasCoach;
    ...
    <div className="hh-dash-root">
          ...
          >
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                ...
                <div>
                  ...
                  >
                    {selectedCoach.specialization || "Fitness Coach"}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div style={{ padding: 24 }}>
              ...
              >
                <div>
                  ...
                  <p style={{ margin: 0, fontSize: 14 }}>
                    {selectedCoach.experience_years || "—"} years
                  </p>
                </div>
                <div>
                  <h4
                ...
                <div style={{ marginBottom: 20 }}>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {selectedCoach.availability.map((a, i) => (
                      ...
                      >
                        {a.day_of_week} {a.start_time.substring(0, 5)}-
              ...
              >
                ...
                {/* Alert dentro del modal si ya tiene pending o coach */}
                {isBlocked ? (
                  <div
                    style={{
                      padding: "12px 16px",
                      borderRadius: 8,
                      backgroundColor: "rgba(var(--hh-accent-rgb), 0.08)",
                  ...
                  >
                    ...
                    >
                      {hasCoach
                        ? "You already have an active coach. Unlink your current coach before sending a new request."
                        : `You already have a pending request with ${pendingRequest?.coach_name}. Wait for their response before sending another.`}
                    </p>
                  </div>
                ) : (
                  <textarea
                ...
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: isBlocked
                      ? "var(--hh-border)"
                      : "var(--hh-accent)",
                    color: isBlocked ? "var(--hh-text-muted)" : "white",
                    fontSize: 14,
                    fontWeight: 600,
      ...
      <main className="hh-dash-main">
        <div className="hh-dash-content">
          ...
          {/* Alert global de solicitud pendiente */}
          {!loading && pendingRequest && (
            ...
            >
              ...
              <div>
                ...
                >
                  Sent to <strong>{pendingRequest.coach_name}</strong> on{" "}
                  {new Date(pendingRequest.requested_at).toLocaleDateString()}.
                  Wait for their response before sending another.
                </p>
              </div>
            </div>
          )}
          ...
          {!loading && hasCoach && (
            <div
              style={{
                padding: "14px 20px",
                borderRadius: 8,
                backgroundColor: "rgba(var(--hh-text-green-rgb), 0.08)",
                border: "1px solid var(--hh-text-green)",
                display: "flex",
                alignItems: "center",
                gap: 12,
              }}
            >
              ...
              >
                Ya tienes un coach activo. Debes desvincularte antes de buscar
                otro.
              </p>
            </div>
          )}
              ...
              {filtered.map((coach) => (
                ...
                >
                  <div style={{ padding: 20 }}>
                    ...
                    >
                      <span
                        style={{ fontSize: 12, color: "var(--hh-text-muted)" }}
                      >
